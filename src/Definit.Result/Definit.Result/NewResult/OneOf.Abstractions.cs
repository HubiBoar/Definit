...
{
    public T Match<T>(Func<T0, T> onT0, Func<T1, T> onT1, Func<Exception, T> onException);
    public void Switch(Action<T0> onT0, Action<T1> onT1, Action<Exception> onException);
}
